...
To use the OpenAI embedding models on other platforms such as Azure, you can use the `api_base` and `api_type` parameters. In this case you have to pass the deployment name of your deplyoed OpenAI embeddings model as `model_name`: 
```python
openai_ef = embedding_functions.OpenAIEmbeddingFunction(
                api_key="YOUR_API_KEY",
